...
//    }
    ...
    //Save data using the Model Mapper
    ...
    public ProductResponceDTO saveProduct(ProductRequestDTO productRequestDTO)
    {
        ...
        product.setPassword(passwordEncoder.encode(productRequestDTO.getPassword()));
        Product saveedProduct =productDAO.save(product);
        return modelMapper.map(saveedProduct, ProductResponceDTO.class);
    }
    //Get data Using the Model Mapper class

    @Override
    public ProductResponceDTO getProduct(Long id) {
        Product product = productDAO.findById(id);
        ProductResponceDTO dto=modelMapper.map(product,ProductResponceDTO.class);
        return dto;
    }
    //Update using the Model Mapper

    @Override
    public ProductResponceDTO updateProduct(ProductRequestDTO productRequestDTO, Long id) {
        //here we are cheaking the id is presnt or not
        Product product=productDAO.findById(id);
        ...
        return modelMapper.map(savedProduct,ProductResponceDTO.class);
    }
